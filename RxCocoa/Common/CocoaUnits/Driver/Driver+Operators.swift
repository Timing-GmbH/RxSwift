...
extension Collection where Iterator.Element : DriverConvertibleType {
    ...
    */
    // @warn_unused_result(message:"http://git.io/rxs.uo")
    public func combineLatest<R>(debounceDependencies: Bool = false, _ resultSelector: @escaping ([Generator.Element.E]) throws -> R) -> Driver<R> {
        let source = self.map { $0.asDriver().asObservable() }.combineLatest(debounceDependencies: debounceDependencies, resultSelector)
        return Driver<R>(source)
    }
}
